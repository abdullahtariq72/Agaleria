...
open class ImageViewZoom: UIScrollView {
    ...
    @objc open func display(image: UIImage) {
        ...
        if (imgSize.width < UIScreen.main.bounds.width) {
            imgSize.width = UIScreen.main.bounds.width
            
            let imgRatio = UIScreen.main.bounds.width/image.size.width
            let expectedHeight = imgSize.height * imgRatio
            
            imgSize.height = expectedHeight
        }
        
        zoomView = UIImageView(image: image)
        if (zoomView!.frame.size.width < UIScreen.main.bounds.width) {
            zoomView!.frame = CGRect.init(x: 0, y: 0, width: imgSize.width, height: imgSize.height)
